...
<body>
    ...
    if (isset($_POST['submit'])) {
        echo "Form submitted!"; // Debug: Form submission check
        ...
        if (!empty(trim($name)) && !empty(trim($username)) && !empty(trim($email)) && !empty(trim($password)) && !empty(trim($repass))) {
            ...
            if ($password == $repass) {
                ...
                if (cek_nama($name, $conn) == 0) {
                    ...
                    if ($result) {
                        echo "User registered successfully!"; // Debug: Registration success
                        // $_SESSION['username'] = $username; // Remove this line
